        ...
        try
        {
            ...
            foreach (var dateStr in unsyncedDates)
            {
                ...
                var data = new Dictionary<string, object>
                {
                    ...
                    ["total_hotkey_presses"] = summary.TotalHotkeyPresses,
                    ["total_filter_changes"] = summary.TotalFilterChanges,
                    ["total_clipboard_copies"] = summary.TotalClipboardCopies,
                    ["total_errors"] = summary.TotalErrors,
                    ["total_tags_created"] = summary.TotalTagsCreated,
                    ["total_tags_updated"] = summary.TotalTagsUpdated,
                    ["total_tag_searches"] = summary.TotalTagSearches,
                    ["total_tag_carousel_clicks"] = summary.TotalTagCarouselClicks,
                    ["total_tasks_deleted"] = summary.TotalTasksDeleted,
                    ["total_setting_changes"] = summary.TotalSettingChanges,
                    ["total_smart_search_filter_uses"] = summary.TotalSmartSearchFilterUses,
                    ["avg_startup_timing_ms"] = summary.AvgStartupTimingMs,
                    ["avg_search_timing_ms"] = summary.AvgSearchTimingMs,
                    ["avg_search_result_click_position"] = summary.AvgSearchResultClickPosition,
                    ["total_search_result_clicks"] = summary.TotalSearchResultClicks,
        ...
        Dictionary<string, (string userName, string deviceName)> deviceLookup)
    {
        try
        {
            ...
            return new DailyMetricsSummary
            {
                ...
                TotalTagsCreated = GetInt(fields, "total_tags_created"),
                TotalTagsUpdated = GetInt(fields, "total_tags_updated"),
                TotalTagSearches = GetInt(fields, "total_tag_searches"),
                TotalTagCarouselClicks = GetInt(fields, "total_tag_carousel_clicks"),
                TotalTasksDeleted = GetInt(fields, "total_tasks_deleted"),
                TotalSettingChanges = GetInt(fields, "total_setting_changes"),
                TotalSmartSearchFilterUses = GetInt(fields, "total_smart_search_filter_uses"),
                AvgStartupTimingMs = fields.TryGetValue("avg_startup_timing_ms", out var ast) && ast is double astD ? astD : 0,
                AvgSearchTimingMs = fields.TryGetValue("avg_search_timing_ms", out var asrch) && asrch is double asrchD ? asrchD : 0,
                AvgSearchResultClickPosition = fields.TryGetValue("avg_search_result_click_position", out var asrc) && asrc is double asrcD ? asrcD : 0,
                TotalSearchResultClicks = GetInt(fields, "total_search_result_clicks"),
            };
        }
        catch
        {
