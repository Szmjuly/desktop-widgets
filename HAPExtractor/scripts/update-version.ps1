#!/usr/bin/env pwsh
# Quick version updater for HAPExtractor
# Usage: .\update-version.ps1 "1.0.0" "Initial release"
#        .\update-version.ps1 "1.0.0" "Bug fixes" -ServiceAccountPath "path\to\sa.json"
...
$candidates += @(
    ...
    (Join-Path $PWD.Path "HAPExtractor\secrets\firebase-license.json"),
...
$data = @{
    ...
    download_url = "https://github.com/Szmjuly/desktop-widgets/releases/download/v$Version/HAPExtractor.exe"
    required_update = $false
    updated_at = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
} | ConvertTo-Json

$url = "$baseUrl/app_versions/hapextractor.json?access_token=$token"

Write-Host "Updating HAPExtractor to v$Version..." -ForegroundColor Cyan
